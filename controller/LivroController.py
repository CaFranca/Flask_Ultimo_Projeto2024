...
def add_book():
    try:
        ...
        quantidade_total=request.form.grt('quantidade_total')
...
def edit_book(id):
    if request.method == 'POST':
        try:
            ...
            if not (titulo and isbn and data_publicacao  and autor_id and categoria_id):
                flash("Todos os campos são obrigatórios.", "error")
                return redirect(url_for('bp_books.edit_book', id=id))

            resultado = livroRepository.updateBook(
                id, titulo, isbn, data_publicacao, autor_id, categoria_id
            )
